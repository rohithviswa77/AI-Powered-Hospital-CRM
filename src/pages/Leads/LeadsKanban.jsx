...
const COLUMNS = [
  { id: 'New Enquiry', title: 'New Enquiry', color: 'border-blue-500', bg: 'bg-blue-50/50' },
  { id: 'Contacted', title: 'Contacted', color: 'border-yellow-500', bg: 'bg-yellow-50/50' },
  { id: 'Converted', title: 'Converted', color: 'border-emerald-500', bg: 'bg-emerald-50/50' },
...
export default function LeadsKanban({ leads, onDragEnd, onLeadClick, isDragDisabled = false }) {
  ...
  const columnsData = COLUMNS.reduce((acc, col) => {
    let stageLeads = leads.filter(lead => lead.lifeStage === col.id);
                      ...
                      <Draggable key={lead.id} draggableId={lead.id} index={index} isDragDisabled={isDragDisabled}>
                        {(provided, snapshot) => (
                          ...
                          >
                            ...
                            <h4 className="text-sm font-bold text-neutral-800 mb-0.5 leading-tight">{`${lead.firstName} ${lead.lastName}`}</h4>
                            
                            <p className="text-[10px] font-semibold text-primary-600 mb-2 truncate">{lead.treatmentSought || lead.leadCategory || 'Enquiry'}</p>
                            ...
                            <div className="flex flex-col gap-1 pt-2 border-t border-neutral-100">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="w-6 h-6 rounded-full bg-neutral-200 border-2 border-white flex items-center justify-center text-[10px] font-bold text-neutral-600 shadow-sm" title={`Assigned to ${lead.assignedTo || 'Unassigned'}`}>
