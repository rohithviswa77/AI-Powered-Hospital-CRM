...
import PatientsKanban from './PatientsKanban';
...
const Patients = () => {
  ...
  const handleDragEnd = async (result) => {
    ...
    setPatients(prev => prev.map(p => p.id === draggableId ? { ...p, treatmentStage: destination.droppableId } : p));

    try {
      const patientRef = doc(db, "patients", draggableId);
      await setDoc(patientRef, { treatmentStage: destination.droppableId }, { merge: true });
    ...
    <div className="animate-fade-in w-full max-w-full overflow-hidden">
          ...
          <div className="flex bg-neutral-100 p-1 rounded-lg border border-neutral-200">
            <button
              ...
              className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors flex items-center gap-1.5 ${
                viewMode === 'board' ? 'bg-white shadow-sm text-primary-700' : 'text-neutral-500 hover:text-neutral-800'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors flex items-center gap-1.5 ${
                viewMode === 'list' ? 'bg-white shadow-sm text-primary-700' : 'text-neutral-500 hover:text-neutral-800'
              }`}
            >
      ...
      {viewingProfile && (
        <PatientProfileDrawer
          ...
          onDeletePatient={(patient) => setDeletingPatientId(patient.id)}
        />
      )}
      ...
      {viewMode === 'list' && (
        <DeleteConfirmationModal
          ...
          message="Are you sure you want to delete this patient? All their medical history, complaints, and outreach logs will be disconnected from the view permanently."
          onConfirm={() => handleDelete(deletingPatientId)}
      ...
      {viewMode === 'board' && (
        <DeleteConfirmationModal
          ...
          message="This will permanently delete this patient from the treatment board. This action cannot be undone."
          onConfirm={() => handleDelete(deletingPatientId)}
          onCancel={() => setDeletingPatientId(null)}
        />
      )}
